...
import { Search, Plus, X, Edit2, Trash2, Shield } from 'lucide-react';
import { cn } from '@/utils/cn';
import { Filter } from '@/components/common/Filter';

const roleInitials = (name: string) => name ? name.slice(0, 2).toUpperCase() : 'RO';
...
export const RoleSplitView = ({
    ...
    const filtered = useMemo(() => (roles || []).filter((r: any) => {
                    ...
                    <DataTable
                        ...
                        columns={[
                            ...
                            },
                            {
                                ...
                                className: 'w-[40%] min-w-[200px] py-4 px-6 text-left text-slate-600 dark:text-slate-300',
                                render: (role: any) => (
                            ...
                            },
                            {
                                ...
                                render: (cat: any) => <StatusBadge isActive={cat.active} />
                            },
                            {
                                header: 'Actions',
                        ...
                        onRowClick={(cat: any) => { setSelectedRole(cat); setTab("general"); }}
    ...
    const renderSplitView = () => {
        const colorInfo = getColorForRole(selectedRole?.id || 0);
                ...
                <div className="w-[320px] min-w-[320px] bg-white dark:bg-gray-900 border-r border-slate-200 dark:border-gray-800 flex flex-col overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto p-2 scrollbar-thin">
                        ...
                        {filtered.map((cat: any) => {
                            const itemColor = getColorForRole(cat.id);
                            const isSelected = selectedRole?.id === cat.id;
                            return (
                                <div
                                    key={cat.id}
                                    onClick={() => { setSelectedRole(cat); setTab("general"); }}
                                    className={cn(
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="text-xs text-slate-400 dark:text-slate-500 truncate">{cat.description ? `${cat.description.substring(0, 20)}...` : '-'}</div>
                                    </div>
                                    <div className={cn(
                                        "w-2 h-2 rounded-full shrink-0 shadow-sm",
                                        cat.active ? "bg-emerald-500" : "bg-slate-300 dark:bg-slate-600"
                                    )} />
                                </div>
                            );
                        })}
                ...
                <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-gray-900">
                    {selectedRole ? (
                            ...
                            <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100 dark:border-gray-800 bg-white dark:bg-gray-900">
                                <div className="flex items-center gap-5">
                                    <div className={cn(
                                        ...
                                        {roleInitials(selectedRole.roleName)}
                                    </div>
                                    <div>
                                ...
                                <div className="flex items-center gap-2.5">
                                    <button onClick={() => handleOpenModal(selectedRole)} className="flex items-center gap-1.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/40 border-none rounded-lg px-4 py-2 text-[13px] font-semibold transition-colors">
                                        <Edit2 className="w-3.5 h-3.5" /> Edit
                                    </button>
                                    <button onClick={() => handleDeleteClick(selectedRole.id)} className="flex items-center gap-1.5 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 border-none rounded-lg px-4 py-2 text-[13px] font-semibold transition-colors">
                                        <Trash2 className="w-3.5 h-3.5" /> Delete
                                    </button>
                                    <div className="w-px h-6 bg-slate-200 dark:bg-gray-700 mx-1"></div>
                                    <button onClick={() => setSelectedRole(null)} className="bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-lg w-9 h-9 flex items-center justify-center text-slate-500 hover:bg-slate-100 dark:hover:bg-gray-700 transition-colors">
                                        <X className="w-4 h-4" />
                                    </button>
                                </div>
                            </div>
                                    ...
                                    <div className="space-y-6">
                                        <div className="space-y-4">
                                            <div>
                                                ...
                                                <p className="mt-1 text-base text-gray-900 dark:text-gray-100">{selectedRole.description}</p>
                                            </div>
                                            <div>
                                                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Status</h3>
                                                <span className={cn(
                                                    'mt-1 inline-flex px-2 py-1 text-xs font-medium rounded-full',
                                                    selectedRole.active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400'
                                                )}>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 bg-slate-50/50 dark:bg-gray-900/50">
                            ...
                            <div className="text-xl font-bold text-slate-600 dark:text-slate-300 tracking-tight">Select a role</div>
                            <div className="text-[13.5px] mt-2 max-w-xs text-center">Choose a role from the sidebar to view its details.</div>
                        </div>
                    )}
                </div>
            </div>
